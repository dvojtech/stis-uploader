...
# kde EXE skuteÄnÄ› leÅ¾Ã­
EXE_DIR = Path(sys.argv[0]).resolve().parent
# kam urÄitÄ› umÃ­me zapsat
TEMP_DIR = Path(os.environ.get("TEMP", str(EXE_DIR)))
# cesty pro "boot" log (zapisujeme na obÄ› mÃ­sta)
BOOT_FILES = [
    TEMP_DIR / "stis_boot.log",
    EXE_DIR / "stis_boot.log",
]
# --- konfigurace mapovÃ¡nÃ­ Å™Ã¡dkÅ¯ v listu "zdroj" ---
ZDROJ_SHEET             = "zdroj"
ZDROJ_FIRST_SINGLE_ROW  = 7     # prvnÃ­ Å™Ã¡dek singlÅ¯ (D/E = jmÃ©na, Iâ€”M = sety)
SINGLES_COUNT           = 16    # kolik singlÅ¯ se vyplÅˆuje (2..17)

def wait_online_ready(page, log, timeout=25000):
    """
    PoÄkÃ¡ na naÄtenÃ­ online editoru.
    """
    try:
        # PoÄkej na zÃ¡kladnÃ­ strukturu
        ...
        log(f"Online editor se nenaÄetl: {repr(e)}")
        raise

def _fill_player_by_click(page, selector, name, log):
    """
    Klikne na .player-name element a vyplnÃ­ jmÃ©no pÅ™es autocomplete.
    """
    name = (name or "").strip()
    if not name:
        return
    ...
    try:
        ...
        page.wait_for_timeout(300)  # KrÃ¡tkÃ¡ pauza pro aktivaci autocomplete
        
        # 2) Najdi aktivnÃ­ autocomplete input
        ...
        for sel in autocomplete_selectors:
            ...
            if inp.count():
                inp.fill(name)
                page.keyboard.press("Tab")  # Nebo Enter pro potvrzenÃ­
                page.wait_for_timeout(200)
                log(f"  âœ“ {name} â†’ {selector}")
                input_found = True
                break
                
        if not input_found:
            # Fallback: zkus napsat do aktuÃ¡lnÄ› fokusovanÃ©ho elementu
            page.keyboard.type(name)
            page.keyboard.press("Tab")
            log(f"  ~ {name} â†’ {selector} (fallback)")
            
    except Exception as e:
        log(f"  âœ— {name} â†’ {selector} failed: {repr(e)}")

def _fill_sets_by_event_index(page, event_index, sets, log):
    """
    VyplnÃ­ sety pro danÃ½ event podle jeho pozice v seznamu.
    """
    if not sets:
        return
    ...
    try:
        ...
        for i, value in enumerate(sets[:5]):
            ...
            if set_input.count():
                ...
                log(f"  set{i+1} â† {value} (event #{event_index})")
            else:
...
def _map_wo(val):
    """Mapuje WO znaÄky na STIS kÃ³dy."""
    if not val:
        return ""
...
def _dom_dump(page, xlsx_path, log):
    try:
        ...
        log(f"DOM dump â†’ {html_path.name}, screenshot â†’ {png_path.name}")
    except Exception as e:
        log(f"DOM dump failed: {repr(e)}")

def cnt(page, css):  # krÃ¡tkÃ¡ pomÅ¯cka do logu
    try:
...
def map_wo(s):
    """mapovÃ¡nÃ­ WO znaÄek na STIS kÃ³dy, jinak vracÃ­ ÄistÃ½ text/ÄÃ­slo"""
    t = str(s or "").strip()
    if not t:
        return ""
...
def read_zdroj_data(xlsx_path):
    """NaÄte ÄtyÅ™hru a singly z listu 'zdroj' â€“ podle vÃ½Å¡e uvedenÃ½ch konstant."""
    from openpyxl import load_workbook
    wb = load_workbook(xlsx_path, data_only=True)
    if ZDROJ_SHEET not in wb.sheetnames:
        raise RuntimeError(f"V seÅ¡itu chybÃ­ list '{ZDROJ_SHEET}'")
    sh = wb[ZDROJ_SHEET]

    def cell(r, c):
        return str(sh.cell(r, c).value or "").strip()

    # ÄŒTYÅ˜HRA â€“ jmÃ©na (D2,D3,E2,E3) a sety (I3..M3)
    double = {
        "home1": cell(2, 4),  # D2
        "home2": cell(3, 4),  # D3
        "away1": cell(2, 5),  # E2
    ...
    # SINGLY â€“ indexy 2..(2+SINGLES_COUNT-1) â†’ Å™Ã¡dky od ZDROJ_FIRST_SINGLE_ROW
    singles = []
...
def fill_online_from_zdroj(page, data, log, xlsx_path=None):
    """
    VyplnÃ­ online formulÃ¡Å™ STIS podle skuteÄnÃ© struktury DOM.
    """
    try:
        wait_online_ready(page, log)
    except Exception:
        log("Inputs se neobjevily â€“ dÄ›lÃ¡m dump DOMu.")
        if xlsx_path:
    ...
    # Sety pro ÄtyÅ™hru #1 (prvnÃ­ .event)
    if dbl.get("sets"):
        _fill_sets_by_event_index(page, 0, dbl["sets"], log)

    # ---- SINGLY (d0 aÅ¾ d15) ----
    # VaÅ¡e data majÃ­ indexy 2-17, ale DOM mÃ¡ d0-d15, takÅ¾e index-2 = DOM_ID
    for match_data in data.get("singles", []):
        ...
        event_idx = excel_idx    # pozice v seznamu eventÅ¯ (ÄtyÅ™hry zabÃ­rajÃ­ 0,1, singly zaÄÃ­najÃ­ od 2)
    ...
    # UloÅ¾it zmÄ›ny
    log("KlikÃ¡m 'UloÅ¾it zmÄ›ny'â€¦")
    try:
        page.locator("input[name='ulozit']").click(timeout=5000)
        page.wait_for_timeout(1000)
        log("ZmÄ›ny uloÅ¾eny.")
    except Exception as e:
        log(f"UloÅ¾enÃ­ selhalo: {repr(e)}")

def boot(msg: str):
    """ZapÃ­Å¡ krÃ¡tkou zprÃ¡vu jeÅ¡tÄ› pÅ™ed main() â€“ pÅ™eÅ¾ije i selhÃ¡nÃ­ argparse."""
...
def make_logger(xlsx_path: Path):
    """VrÃ¡tÃ­ (log_fn, file_handle, log_path) â€“ loguje s Äasovou znaÄkou."""
...
def ensure_pw_browsers(log=None):
    """StÃ¡hne Chromium pro Playwright, pokud chybÃ­ (funguje i z PyInstaller EXE)."""
    ...
    if log: log("Chromium not found â€“ starting Playwright install (can take minutes)â€¦")
...
def norm(x) -> str:
    ...
    # bez mezer a nealfanumerickÃ½ch znakÅ¯
    s = re.sub(r"\s+", "", s)
...
def get_setup_sheet(wb):
    """Najdi list 'setup' case-insensitive, jinak vraÅ¥ prvnÃ­ list."""
...
def find_login_pwd(ws):
    """VrÃ¡tÃ­ login/heslo. Nejprve z B1/B2, kdyÅ¾ chybÃ­, zkusÃ­ popisky 'login' / 'heslo' v hornÃ­ ÄÃ¡sti listu."""
...
def find_teams_header_anywhere(wb):
    """
    Projdi vÅ¡echny listy a najdi prvnÃ­ Å™Ã¡dek, kde je hlaviÄka tabulky Teams:
      - obsahuje 'druzstvo' (ale ne 'id' / 'vedouci')
      - a zÃ¡roveÅˆ nÄ›kterÃ½ JASNÃ marker ID ('druzstvoid', 'id_druzstva', 'iddruzstva') nebo PÅ˜ESNÄš 'id'
    VracÃ­ (sheet, hdr_row) nebo (None, None).
    """
    strict_id_markers = ("druzstvoid", "id_druzstva", "iddruzstva")  # Å¾Ã¡dnÃ© volnÃ© "id" jako podÅ™etÄ›zec!
    for ws in wb.worksheets:
...
def open_match_form(page, log):
    """Na strÃ¡nce druÅ¾stva otevÅ™e formulÃ¡Å™ â€“ preferuje 'vloÅ¾it zÃ¡pis', jinak 'upravit zÃ¡pis'.
       ZkouÅ¡Ã­ text i href (zapis_start.php / online.php). VracÃ­ True/False.
    """
    try:
        page.wait_for_selector(
            "a:has-text('vloÅ¾it zÃ¡pis'), a:has-text('upravit zÃ¡pis'), "
            "a[href*='zapis_start.php?u='], a[href*='online.php?u=']",
            timeout=15000
        )
    except Exception:
        log("Nenalezl jsem Å¾Ã¡dnÃ½ z oÄekÃ¡vanÃ½ch odkazÅ¯ do 15 s.")
        return False

    candidates = [
        page.get_by_role("link", name=re.compile(r"vloÅ¾it\s*zÃ¡pis", re.I)),
        page.get_by_role("link", name=re.compile(r"upravit\s*zÃ¡pis", re.I)),
    ...
    for i, sel in enumerate(candidates, 1):
        try:
            if sel.count():
                log(f"ZkouÅ¡Ã­m selector {i}")
                sel.first.click(timeout=5000)
                page.wait_for_load_state("domcontentloaded")

                if page.locator("text=/vklÃ¡dÃ¡nÃ­ zÃ¡pisu/i").count() \
                   or "online.php?u=" in page.url \
                   or "zapis_start.php?u=" in page.url:
                    log("FormulÃ¡Å™ otevÅ™en na URL:", page.url)
                    return True

                if page.locator("text=/Å¡patn.*url/i").count():
                    log("Server hlÃ¡sÃ­ 'Å¡patnÃ© URL' â€“ zkusÃ­m jinÃ½ odkaz.")
                    page.go_back()
                    page.wait_for_load_state("domcontentloaded")
        except Exception as e:
            log(f"Selector {i} selhal:", repr(e))

    # poslednÃ­ zÃ¡chrana â€“ proskenuj vÅ¡echny <a> a skoÄ pÅ™Ã­mo na vyhovujÃ­cÃ­ href
    try:
        ...
        for a in anchors:
            if re.search(r"(zapis_start\.php|online\.php)\?u=\d+", a.get("href",""), re.I):
                log("Jdu pÅ™Ã­mo na", a["href"])
                page.goto(a["href"], wait_until="domcontentloaded")
                return True
    except Exception as e:
        log("Fallback scan anchorÅ¯ selhal:", repr(e))

    return False

def read_excel_config(xlsx_path: Path, team_name: str):
    """
    Najde list s tabulkou Teams kdekoli v seÅ¡itu, pÅ™ihlÃ¡Å¡enÃ­ bere z B1/B2
    (nebo z popiskÅ¯ 'login'/'heslo'), namapuje sloupce a vrÃ¡tÃ­ login, heslo a dict tÃ½mu.
    """
    wb = load_workbook(xlsx_path, data_only=True)

    # 1) Najdi list a Å™Ã¡dek hlaviÄky Teams kdekoli v seÅ¡itu
    setup, hdr_row = find_teams_header_anywhere(wb)
    if setup is None:
        # diagnostika â€“ co jsme nahoÅ™e vidÄ›li
        try:
            ...
            pass
        raise RuntimeError("V setup/Teams chybÃ­ sloupce 'DruÅ¾stvo' a/nebo 'DruzstvoID'.")

    # 2) Login a heslo (z nalezenÃ©ho listu)
    login, pwd = find_login_pwd(setup)
    if not login or not pwd:
        raise RuntimeError("VyplÅˆ login/heslo (B1/B2, nebo vedle popiskÅ¯ 'login'/'heslo').")

    # 3) Namapuj sloupce podle hlaviÄky (ID sloupec detekuj STRIKTNÄš + vyluÄ 'vedouci*')
    ...
    if "name" not in idx or "id" not in idx:
        raise RuntimeError("V Teams chybÃ­ sloupce 'DruÅ¾stvo' a/nebo 'DruzstvoID'.")

    # 4) Najdi poÅ¾adovanÃ© druÅ¾stvo pod hlaviÄkou
    team = None
    r = hdr_row + 1
    ...
    if not team:
        raise RuntimeError(f"DruÅ¾stvo '{team_name}' nenalezeno v Teams.")
    if not team["id"]:
        raise RuntimeError("PrÃ¡zdnÃ© DruzstvoID u zvolenÃ©ho druÅ¾stva.")

    # 5) OÄisti ID na ÄistÃ© ÄÃ­slo (zabrÃ¡nÃ­me tomu, aby se do URL dostalo 'Kozel Petr' apod.)
    raw_id = str(team.get("id", "")).strip()
    m = re.search(r"\d+", raw_id)
    if not m:
        raise RuntimeError(f"NeplatnÃ© DruzstvoID: {raw_id!r}")
    team["id"] = m.group(0)
...
def parse_args():
    ...
    p.add_argument("--xlsx", required=True, help="plnÃ¡ cesta k XLSX")
    p.add_argument("--team", required=True, help="nÃ¡zev druÅ¾stva (sloupec 'DruÅ¾stvo')")
    g = p.add_mutually_exclusive_group()
    g.add_argument("--headed",  dest="headed",  action="store_true",  help="viditelnÃ½ prohlÃ­Å¾eÄ")
    g.add_argument("--headless", dest="headed", action="store_false", help="bez UI")
    p.set_defaults(headed=True)  # vÃ½chozÃ­ = viditelnÃ© okno
    return p.parse_args()
...
def main():
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            # 2) spuÅ¡tÄ›nÃ­ prohlÃ­Å¾eÄe (Chromium â†’ Chrome â†’ Edge)
            log("Launching browserâ€¦ headless =", headless)
            browser = None
            try:
                browser = p.chromium.launch(headless=headless)
                log("Launched: managed Chromium")
            except Exception as e1:
                log("Chromium failed:", repr(e1), "â†’ trying channel=chrome")
                try:
                    browser = p.chromium.launch(channel="chrome", headless=headless)
                    log("Launched: channel=chrome")
                except Exception as e2:
                    log("Chrome failed:", repr(e2), "â†’ trying channel=msedge")
            ...
            # 5) najdi vstup do formulÃ¡Å™e (vloÅ¾it/upravit)
            log("HledÃ¡m odkaz 'vloÅ¾it/upravit zÃ¡pis'â€¦")
            if not open_match_form(page, log):
                raise RuntimeError("Na strÃ¡nce druÅ¾stva jsem nenaÅ¡el odkaz do formulÃ¡Å™e.")

            # 6) vyplÅˆ ÃºvodnÃ­ Ãºdaje (herna / zaÄÃ¡tek / vedoucÃ­)
            if team.get("herna") and page.locator("input[name='zapis_herna']").count():
                page.fill("input[name='zapis_herna']", str(team["herna"]))
                log("Herna vyplnÄ›na:", team["herna"])

            # OPRAVENÃ‰ vyplnÄ›nÃ­ Äasu pomocÃ­ dvou select elementÅ¯
            if team.get("zacatek"):
                try:
                    hh, mm = team["zacatek"].split(":")
                    if page.locator("select[name='zapiszacatekhodiny']").count():
                        page.select_option("select[name='zapiszacatekhodiny']", value=str(int(hh)))
                        log(f"Hodina nastavena: {hh}")
                    if page.locator("select[name='zapiszacatekminuty']").count():
                        page.select_option("select[name='zapiszacatekminuty']", value=str(int(mm)))
                        log(f"Minuta nastavena: {mm}")
                    log(f"Začátek nastaven: {team['zacatek']}")
                except Exception as e:
                    log("Set start time failed:", repr(e))


            # OPRAVENÃ‰ vyplnÄ›nÃ­ vedoucÃ­ch pomocÃ­ autocomplete inputÅ¯
            if team.get("ved_dom"):
                try:
                    ...
                    if ved_input.count():
                        ...
                        log("VedoucÃ­ domÃ¡cÃ­ch:", team["ved_dom"])
                except Exception as e:
                    log("VedoucÃ­ domÃ¡cÃ­ch selhal:", repr(e))
            ...
            if team.get("ved_host"):
                try:
                    ...
                    if ved_input.count():
                        ...
                        page.keyboard.press("Tab")  # Aktivace autocomplete
                        page.wait_for_timeout(500)
                        log("VedoucÃ­ hostÅ¯:", team["ved_host"])
                except Exception as e:
                    log("VedoucÃ­ hostÅ¯ selhal:", repr(e))

            # 7) UloÅ¾it a pokraÄovat (robustnÄ›)
            log("Click 'UloÅ¾it a pokraÄovat'â€¦")
            clicked = False
            
            # Zkus najÃ­t tlaÄÃ­tko "UloÅ¾it a pokraÄovat" podle name atributu z HTML dumpu
            try:
                btn = page.locator("input[name='odeslat']")  # "UloÅ¾it a pokraÄovat" mÃ¡ name="odeslat"
                if btn.count():
                    btn.click(timeout=5000)
                    clicked = True
                    log("Kliknuto na 'UloÅ¾it a pokraÄovat' (name=odeslat)")
            except Exception as e:
            ...
            if not clicked:
                try:
                    btn = page.locator("input[value*='pokraÄovat']")
                    if btn.count():
                        btn.first.click(timeout=5000)
                        clicked = True
                        log("Kliknuto na 'UloÅ¾it a pokraÄovat' (value)")
                except Exception as e:
                    log("Button(value) click failed:", repr(e))

            if not clicked:
                raise RuntimeError("NenaÅ¡el jsem tlaÄÃ­tko/odkaz 'UloÅ¾it a pokraÄovat'.")

            # 8) ÄŒekej na online formulÃ¡Å™
            try:
                page.wait_for_url(re.compile(r"/online\.php\?u=\d+"), timeout=20000)
            except Exception:
                # fallback: aspoÅˆ poÄkat na nÄ›jakÃ© textovÃ© inputy
                page.wait_for_selector("input[type='text']", timeout=10000)
            log("Online formulÃ¡Å™ naÄten:", page.url)
            
            # 9) vyplÅˆ data
            data = read_zdroj_data(xlsx_path)
            fill_online_from_zdroj(page, data, log, xlsx_path)
            
            # 10) vizuÃ¡lnÃ­ dokonÄenÃ­
            if headed:
                log("Leaving browser open for manual finish.")
                print("âœ… Online formulÃ¡Å™ naÄten â€“ dokonÄi ruÄnÄ›. Okno nechÃ¡vÃ¡m otevÅ™enÃ©.")
                while True:
                    time.sleep(1)
            else:
                context.close()
        ...
        try:
            os.startfile(str(log_path))  # otevÅ™Ã­t log v Notepadu
        except Exception:
            pass
        raise
    finally:
...
if __name__ == "__main__":
    ...
    try:
        ...
        boot(f"SystemExit (pravdÄ›podobnÄ› argparse): code={getattr(e, 'code', None)}")
        msgbox("SpuÅ¡tÄ›nÃ­ skonÄilo hned na zaÄÃ¡tku (Å¡patnÃ©/neÃºplnÃ© argumenty?).\n" +
               "Zkontroluj prosÃ­m volÃ¡nÃ­ z Excelu.\n" +
